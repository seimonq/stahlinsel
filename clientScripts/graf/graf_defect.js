...
//		alert(graf.ic.toSource());
	...
	constructor(id,kante1){		//	================================================================
		...
		this.beKannte(graf.knoten[graf.kanten[kante1].parent_id]);  			
		this.beKannte(graf.knoten[graf.kanten[kante1].child_id]);
	...
	beKannte(whichNode){
		...
		if(whichNode.gruppe==NONE){
			whichNode.gruppe=this.id;
			this.not[this.nNot++]=whichNode;

			for(curKante=0;curKante<graf.kantenLength;curKante++){
				if(graf.kanten[curKante].gruppe==NONE){
					if(graf.kanten[curKante].parent_id==whichNode.id){
						graf.kanten[curKante].gruppe=this.id;
						this.kant[this.nKant++]=graf.kanten[curKante];
						this.beKannte(graf.kanten[curKante].child_id);
					}
					if(graf.kanten[curKante].child_id==whichNode.id){
						graf.kanten[curKante].gruppe=this.id;
						this.kant[this.nKant++]=graf.kanten[curKante];
